...
from ..data.dataseek import DataSeek
...
class Database:
    ...
    def __init__(self, _seek_file_location, _data_directory):
        ...
        self._data_seek = DataSeek(self._seek_file_location)
        ...
        if self._data_seek.data_seek_ok() and self._data_directory_validated:
            self._database_ok = True

        if config.debug():
            print('Database Ok: ' + str(self._database_ok))
    ...
    def get_data(self, data_set_obj):
        ...
        for item in data_items:
            seek_location = self._data_seek.get_seek_location(str(item.value()))
            print('Get: Seek Location: ' + str(seek_location))
            data_count = self._data_seek.read_seek_count(seek_location)
    ...
    def store_data_set(self, data_set_obj):
        ...
        for item in data_items:
            ...
            self._data_seek.write_seek_count(seek_location)
